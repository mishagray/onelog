...
let start = label => timers[label] = Date.now();

let stop = function(label) {
	let duration = Date.now() - timers[label];
	return duration;
};
...
class Logger {
			...
			(method => {
				return Logger.prototype[method] = function(...a) {
					...
					if (method === 'start') {
						...
						let defaultMethod = GLOBAL.onelog._library.defaultLevel();
	...
	suppress(level) { return this.enabled = false; }
	allow(level) { return this.enabled = true; }
}
...
let getCallerFile = function() {
	let frame = stackTrace.get()[2];
	//file = path.basename frame.getFileName()
	let file = frame.getFileName();
	let line = frame.getLineNumber();
	let method = frame.getFunctionName();
...
export function use(clazz, opts) {
...
`
	...
	return Array.from(config.methods).map((method) =>
		(method => {
			return exports[method] = (...a) => _defaultLogger[method](...Array.from(a || []));
		})(method));
}
...
export function get(category) {
	...
	if (GLOBAL.onelog == null) { exports.use(Console); }
	return GLOBAL.onelog._library.get(category);
}

// Support for logule namespaces
export function sub(...namespaces) {
...
export function getLibrary() {
	return (GLOBAL.onelog._library != null ? GLOBAL.onelog._library.getLibrary() : undefined);
}
...
class Log4js extends Library {
	...
	middleware(opts) {
		let category = (opts != null ? opts.category : undefined) || 'Middleware';
		let level = (opts != null ? opts.level : undefined) || this.log4js.levels.INFO;
		// TODO: Allow more options
		return this.log4js.connectLogger(this.log4js.getLogger(category), {level});
	}

	defaultLevel() { return 'info'; }
}
Log4js.initClass();
...
class Logule extends Library {
	...
	middleware(opts) {
		let category = (opts != null ? opts.category : undefined) || 'Middleware';
		let level = (opts != null ? opts.level : undefined) || 'trace';
